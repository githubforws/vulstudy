...
from tasks.modules.compose_tasks import docker_container_run, create_compose_task, create_layout_image_download_task
...
from dockerapi.views.utils_views import get_local_ip
...
def build_compose(request):
    ...
    try:
        ...
        image_info.save()
        image_list = create_compose_task(user_info, image_info, tag, get_request_ip(request))
    except Exception as e:
...
def update_build_compose(request):
    ...
    try:
        ...
        image_list = create_compose_task(user_info, image_infos, tag, get_request_ip(request))
    except Exception as e:
    ...
    try:
        for service in services:
            ...
            if not image_info:
                ...
                image_info.save()
        create_layout_image_download_task(layout_instance, user)
...
def download_official_website_layout(request):
    ...
    user = request.user
    url = "https://vulfocus.cn/api/layoutinfodet?layout_id={}".format(id)
...
def get_official_website_layout(request):
    ...
    获取官网编排场景信息（社区）
    '''
    url = "https://vulfocus.cn/api/get/layoutinfo/"
    try:
        res = requests.get(url, verify=False).content
        req = json.loads(res)
