...
# Vulfocus 项目结构介绍
Vulfocus 是一个漏洞靶场管理平台，采用前后端分离架构。下面详细介绍项目的目录结构和各模块功能。

## 项目整体架构
## 后端结构（vulfocus-api）
后端基于 Django 4.2 + Django REST Framework + Celery 5 构建。

### 核心配置
文件 说明 manage.py Django 管理命令入口 settings.py 项目核心配置（数据库、JWT、Celery、Docker等） urls.py URL 路由配置 celery.py Celery 任务调度配置 requirements.txt Python 依赖管理

### 应用模块 1. dockerapi（核心模块）
漏洞镜像和容器管理的核心模块：
 2. tasks（异步任务）
基于 Celery 的异步任务模块：
 3. user（用户管理）
用户认证与授权模块：
 4. layout_image（场景编排）
Docker Compose 场景编排模块：
 5. network（网络管理）
网络配置管理模块：
 6. notice（通知管理）
系统通知模块：

## 前端结构（vulfocus-frontend）
前端基于 Vue 2 + Element UI + Vue Router + Vuex 构建。

## 数据流架构
## 关键技术栈
层级 技术 后端框架 Django 4.2 + Django REST Framework 认证方式 djangorestframework-simplejwt 异步任务 Celery 5 + Redis 数据库 SQLite（默认）/ MySQL（生产） Docker SDK docker-py 前端框架 Vue 2 + Element UI 状态管理 Vuex 路由 Vue Router HTTP 请求 Axios
