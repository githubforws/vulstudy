...
const shapeNodeLstDataList = ref([])

function dragShapeNode(data, key, event) {
  emit('click', data, key, event)
}

function initToolbarNodes() {
  let initShapeLstData = shapeNodeLstData
  if (!initShapeLstData instanceof Array) {
    initShapeLstData = []
  }
  shapeNodeLstDataList.value = initShapeLstData
}
