<template>
  <div class="app-container">
    <div class="svgHead" v-show="editable" :inline="true">
      <div class="svgHeadItemLst svgToolBarItem">
        <el-tooltip class="item" effect="dark" content="返回" placement="top-start">
          <el-button size="small" style="margin: 0" @click="goBack">
            <el-icon><ArrowLeft /></el-icon>
          </el-button>
        </el-tooltip>
      </div>
      <div class="svgHeadItemLst svgToolBarItem">
        <el-upload class="upload_zip" style="" action="" :http-request="uploadlayout" :show-file-list="false" :before-upload="beforeAvatarUploadLayout"><el-button class="filter-item" size="small" style="margin-right: 10px" type="primary"><el-icon><Upload /></el-icon>上传</el-button></el-upload>
        <el-button size="small"  type="primary" @click="saveTopoJson">
          <el-icon><Download /></el-icon> 保存
        </el-button>
      </div>
    </div>
    <div class="svgMain">
      <v-shapebar @click="dragShapeNode" v-show="isShow" style="min-height: calc(100vh - 140px);"></v-shapebar>
      <div :id="'topoId'+topoId" class="topoWrap" ref="topoWrap">
        <svg class="topoSvg"
             :width="svgAttr.width"
             :height="svgAttr.height"
             @mousedown.stop="mousedownTopoSvg($event)"
             :viewBox="svgAttr.viewX+' '+svgAttr.viewY+' '+svgAttr.width+' '+svgAttr.height"
             :class="{'hand':svgAttr.isHand,'crosshair':svgAttr.isCrosshair}">
          <defs>
            <pattern id="Pattern" x="0" y="0" width="100" height="100" patternUnits="userSpaceOnUse">
              <line :x1="ele.x1" :x2="ele.x2" :y1="ele.y1" :y2="ele.y2" :stroke="ele.color"
                    :stroke-width="ele.strokeWidth" :opacity="ele.opacity" v-for="ele in gridData" :key="ele.id"></line>
            </pattern>
          </defs>
          <defs>
            <filter id="f1" x="0" y="0" width="200%" height="200%" filterUnits="userSpaceOnUse">
              <feOffset result="offOut" in="SourceGraphic" dx="4" dy="4"/>
              <feColorMatrix result="matrixOut" in="offOut" type="matrix"
                             values="0.2 0 0 0 0 0 0.2 0 0 0 0 0 0.2 0 0 0 0 0 1 0"/>
              <feGaussianBlur result="blurOut" in="matrixOut" stdDeviation="2"/>
              <feBlend in="SourceGraphic" in2="blurOut" mode="normal"/>
            </filter>
          </defs>
          <rect fill="url(#Pattern)" :width="svgAttr.width" :height="svgAttr.height"/>
          <g>
            <g
              class="nodesG"
              v-for="(ele,key) in topoData.nodes"
              :class="{isSelect:ele.isSelect,hoverShowConnectorArror:editable}"
              :transform="'translate('+ele.x+','+ele.y+')'"
              :key="ele.id"
              @mouseover.stop="mouseoverNode(key,$event)"
              @mousedown.stop="dragSvgNode(key,$event)"
              @mouseout.stop="mouseoutLeftConnector(key)"
            >
              <rect x="0" y="0" rx="2" ry="2" :width="ele.width" :height="ele.height" class="reactClass"/>
              <text v-if="ele.classType == 'T1'" class="nodeName" x="5" y="15">{{ ele.name }}</text>
              <image class="nodeImg" v-if="ele.classType == 'T1'" :xlink:href="ele.icon" :x="ele.width - 18" :y="3"
                     height="15px" width="15px"/>

              <image class="nodeImg" v-if="ele.classType == 'T2'" :xlink:href="ele.icon" :x="7" :y="7" height="36px"
                     width="36px"/>

              <foreignObject v-if="JSON.stringify(ele.attrs) !=='{}' && ele.type === 'Container'" :width="ele.width-30" :height="ele.height" x="5" y="30" style="text-overflow: ellipsis; fill:#768699;color :#768699;">
                <text x="5" y="30" class="nodeName">镜像名称：{{ele.attrs.name}}</text> <br/>
                <text x="5" y="50" class="nodeName">漏洞名称：{{ele.attrs.vul_name}}</text> <br/>
                <text x="5" y="90" class="nodeName">端口：{{ele.attrs.port}}</text><br/>
                <text x="5" y="70" class="nodeName">是否开放：{{ele.attrs.open}}</text><br/>
              </foreignObject>

              <foreignObject v-if="JSON.stringify(ele.attrs) !=='{}' && ele.type === 'Network'" :width="ele.width-30" :height="ele.height" x="5" y="30" style="text-overflow: ellipsis; fill:#768699;color :#768699;">
                <text x="5" y="30" class="nodeName">网卡名称：{{ele.attrs.name}}</text> <br/>
                <text x="5" y="50" class="nodeName">网关：{{ele.attrs.gateway}}</text> <br/>
                <text x="5" y="90" class="nodeName">子网：{{ele.attrs.subnet}}</text><br/>
              </foreignObject>
              <g class="connectorArror" :class="{'connector':ele.isLeftConnectShow}"
                 :transform="'translate(0,'+ele.height/2+')'">
                <circle r="8" cx="0" cy="0" class="circleColor"></circle>
                <line x1="-3" y1="-5" x2="4" y2="0.5" stroke="#fff"></line>
                <line x1="4" y1="-0.5" x2="-3" y2="5" stroke="#fff"></line>
              </g>
              <g class="connectorArror" :class="{'connector':ele.isRightConnectShow}"
                 :transform="'translate('+ele.width+','+ele.height/2+')'" @mousedown.stop="drawConnectLine(key,$event)">
                <circle r="8" cx="0" cy="0" class="circleColor"></circle>
                <line x1="-3" y1="-5" x2="4" y2="0.5" stroke="#fff"></line>
                <line x1="4" y1="-0.5" x2="-3" y2="5" stroke="#fff"></line>
              </g>
            </g>
            <!-- node间关系连线样式 -->
            <template v-for="(ele,key) in topoData.connectors" :key="ele.id">
              <g
                class="connectorsG"
                :class="{active:ele.isSelect}"
                v-if="ele.type == 'Line'"
                @mousedown.stop="selectConnectorLine(key)"
              >
                <!-- 自连 -->
                <path
                  class="connectorLine"
                  :class="{'defaultStrokeColor':!ele.color,'defaultStrokeW':!ele.strokeW}"
                  :stroke="ele.color"
                  :stroke-width="ele.strokeW"
                  v-if="ele.sourceNode.id == ele.targetNode.id"
                  :d="'M'+(ele.sourceNode.x + ele.sourceNode.width)+','+(ele.sourceNode.y + ele.sourceNode.height / 2)+
                  'h'+connectorWSelf+
                  'v'+(-(ele.sourceNode.height / 2 + connectorWSelf))+
                  'h'+ (-(ele.sourceNode.width +  2 * connectorWSelf)) +
                  'v'+(ele.sourceNode.height / 2 + connectorWSelf) +
                  'H' + (ele.targetNode.x)"
                ></path>
                <!-- 非自连:1.sourceNode 的右侧箭头X <= targetNode的左侧箭头X -->
                <path
                  class="connectorLine"
                  :class="{'defaultStrokeColor':!ele.color,'defaultStrokeW':!ele.strokeW}"
                  :stroke="ele.color"
                  :stroke-width="ele.strokeW"
                  v-if="ele.sourceNode.id != ele.targetNode.id &&
                  (ele.sourceNode.x +ele.sourceNode.width) < ele.targetNode.x"
                  :d="'M'+(ele.sourceNode.x + ele.sourceNode.width)+','+(ele.sourceNode.y + ele.sourceNode.height / 2) +
                  'h'+ (ele.targetNode.x - ele.sourceNode.x - ele.sourceNode.width) / 2 +
                  'V' + (ele.targetNode.y + ele.targetNode.height / 2) +
                  'H' + ele.targetNode.x"
                ></path>
                <!-- 非自连：
                2.sourceNode 的右侧箭头X >= targetNode的左侧箭头X
                (1) 且 sourceNode的高度 < targetNode的高度 且 高度未重叠-->
                <path
                  class="connectorLine"
                  :class="{'defaultStrokeColor':!ele.color,'defaultStrokeW':!ele.strokeW}"
                  :stroke="ele.color"
                  :stroke-width="ele.strokeW"
                  v-if="ele.sourceNode.id != ele.targetNode.id &&
                  (ele.sourceNode.x + ele.sourceNode.width) >= ele.targetNode.x &&
                  (ele.sourceNode.y + ele.sourceNode.height ) < ele.targetNode.y"
                  :d="'M'+(ele.sourceNode.x + ele.sourceNode.width)+','+(ele.sourceNode.y + ele.sourceNode.height / 2) +
                  'h'+connectorWSelf+
                  'v'+(ele.sourceNode.height / 2 + (ele.targetNode.y - ele.sourceNode.y -  ele.sourceNode.height) / 2) +
                  'H'+(ele.targetNode.x - connectorWSelf) +
                  'V'+(ele.targetNode.y + ele.targetNode.height / 2) +
                  'h'+connectorWSelf"
                ></path>
                <!-- 非自连：
                2.sourceNode 的右侧箭头X >= targetNode的左侧箭头X
                  (2) 且 sourceNode的高度 > targetNode的高度 且 高度未重叠-->
                <path
                  class="connectorLine"
                  :class="{'defaultStrokeColor':!ele.color,'defaultStrokeW':!ele.strokeW}"
                  :stroke="ele.color"
                  :stroke-width="ele.strokeW"
                  v-if="ele.sourceNode.id != ele.targetNode.id &&
                  (ele.sourceNode.x + ele.sourceNode.width) >= ele.targetNode.x &&
                  (ele.targetNode.y + ele.targetNode.height) < ele.sourceNode.y"
                  :d="'M'+(ele.sourceNode.x + ele.sourceNode.width)+','+(ele.sourceNode.y + ele.sourceNode.height / 2) +
                  'h'+connectorWSelf+
                  'V'+(ele.sourceNode.y-(ele.sourceNode.y - ele.targetNode.y - ele.targetNode.height) / 2) +
                  'H'+ (ele.targetNode.x - connectorWSelf) +
                  'V'+(ele.targetNode.y + ele.targetNode.height / 2) +
                  'H'+ele.targetNode.x"
                ></path>
                <!--
                非自连：
                2.sourceNode 的右侧箭头X >= targetNode的左侧箭头X
                (3) sourceNode的箭头y < = targetNode的箭头
              sourceNode 的y < targetNode的y < = (sourceNode 的y + sourceNode的height) 或者 sourceNode的y介于其间
                高度重叠-->
                <path
                  class="connectorLine"
                  :class="{'defaultStrokeColor':!ele.color,'defaultStrokeW':!ele.strokeW}"
                  :stroke="ele.color"
                  :stroke-width="ele.strokeW"
                  v-if="ele.sourceNode.id != ele.targetNode.id &&
                  (ele.sourceNode.x + ele.sourceNode.width) >= ele.targetNode.x &&
                  (ele.sourceNode.y + ele.sourceNode.height/2) <= (ele.targetNode.y + ele.targetNode.height / 2) &&
                  ((ele.targetNode.y <= (ele.sourceNode.y + ele.sourceNode.height) && ele.targetNode.y >= ele.sourceNode.y) ||
                  (ele.sourceNode.y <= (ele.targetNode.y + ele.targetNode.height) && ele.sourceNode.y >= ele.targetNode.y)
                  )"
                  :d="'M'+(ele.sourceNode.x + ele.sourceNode.width)+','+(ele.sourceNode.y + ele.sourceNode.height / 2)+'h'+connectorWSelf +
                  'V'+ ((ele.sourceNode.y-ele.targetNode.y ) <= 0? (ele.sourceNode.y - connectorWSelf) : (ele.targetNode.y -connectorWSelf)) +
                  'H' + (ele.targetNode.x - connectorWSelf) +
                  'V' +(ele.targetNode.y + ele.targetNode.height / 2) +
                  'H' + ele.targetNode.x"
                ></path>
                <!--
                非自连：
                2.sourceNode 的右侧箭头X > targetNode的左侧箭头X
                (3) 且 sourceNode的高度 < targetNode的高度 且
                sourceNode的起点 > targetNode的终点 且
                高度重叠-->
                <path
                  class="connectorLine"
                  :class="{'defaultStrokeColor':!ele.color,'defaultStrokeW':!ele.strokeW}"
                  :stroke="ele.color"
                  :stroke-width="ele.strokeW"
                  v-if="ele.sourceNode.id != ele.targetNode.id &&
                  (ele.sourceNode.x + ele.sourceNode.width) >= ele.targetNode.x &&
                  (ele.sourceNode.y + ele.sourceNode.height/2) > (ele.targetNode.y + ele.targetNode.height / 2) &&
                  ((ele.targetNode.y <= (ele.sourceNode.y + ele.sourceNode.height) && ele.targetNode.y >= ele.sourceNode.y) ||
                  (ele.sourceNode.y <= (ele.targetNode.y + ele.targetNode.height) && ele.sourceNode.y >= ele.targetNode.y)
                  )"
                  :d="'M'+(ele.sourceNode.x + ele.sourceNode.width)+','+(ele.sourceNode.y + ele.sourceNode.height / 2)+'h'+connectorWSelf +
                  'V'+ ((ele.sourceNode.y  + ele.sourceNode.height-ele.targetNode.y -ele.targetNode.height ) >= 0? (ele.sourceNode.y+ele.sourceNode.height + connectorWSelf) : (ele.targetNode.y+ele.targetNode.height +connectorWSelf)) +
                  'H' + (ele.targetNode.x - connectorWSelf) +
                  'V' +(ele.targetNode.y + ele.targetNode.height / 2) +
                  'H' + ele.targetNode.x"
                ></path>
              </g>
            </template>
            <!-- 动态绘制的连线 -->
            <g>
              <line :x1='connectingLine.x1' :y1="connectingLine.y1" :x2="connectingLine.x2" :y2="connectingLine.y2"
                    v-show="connectingLine.isConnecting" stroke="#768699" stroke-width="2"></line>
            </g>
          </g>
          <line :class="{isMarkerShow:marker.isMarkerShow}" id="xmarker" class="marker" x1="0" :y1="marker.xmarkerY"
                :x2="marker.xmarkerX" :y2="marker.xmarkerY"></line>
          <line :class="{isMarkerShow:marker.isMarkerShow}" id="ymarker" class="marker" :x1="marker.ymarkerX" y1="0"
                :x2="marker.ymarkerX" :y2="marker.ymarkerY"></line>
          <rect :x="selectionBox.x" :y="selectionBox.y" :width="selectionBox.width" :height="selectionBox.height"
                stroke-dasharray="5,5" stroke-width="1" stroke="#222" fill="rgba(170,210,232,0.5)"
                v-show="selectionBox.isShow"/>
        </svg>
        <v-topo-attr-panel :v-select-node-data="selectNodeData" v-show="editable"></v-topo-attr-panel>
      </div>
    </div>
    <div v-if="shapebarMoveNode.isShow" class="moveNode nodeMoveCss" :style="{ left:shapebarMoveNode.left + 'px', top: shapebarMoveNode.top + 'px' }">
          <div class="shapeIcon">
            <img class="shapeIconImg" :src="shapebarMoveNode.icon"/>
          </div>
          <div class="shapeName">{{shapebarMoveNode.name}}</div>
        </div>
    <el-dialog v-model="editShow" title="新增">
      <el-form label-width="80px" v-loading="editLoading" element-loading-text="新增中">
        <el-form-item label="环境名称">
          <el-input v-model="layout.name" size="medium"></el-input>
        </el-form-item>
        <el-form-item label="环境描述">
          <el-input type="textarea"
                    v-model="layout.desc" size="medium"></el-input>
        </el-form-item>
        <el-form-item label="Banner 图">
          <el-upload
            class="avatar-uploader"
            action=""
            :http-request="upload"
            :show-file-list="false"
            :before-upload="beforeAvatarUpload">
            <img v-if="layout.imageName" :src="'/images/'+layout.imageName" class="avatar">
            <el-icon v-else class="avatar-uploader-icon"><Plus /></el-icon>
          </el-upload>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" size="medium" @click="handleOk">确定</el-button>
          <el-button size="medium" @click="handleCancel">取消</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
    <el-dialog v-model="ymlShow" width="60%">
      <el-tabs model-value="dockerfile" ref="tab">
        <el-tab-pane name="dockerfile">
          <template #label><el-icon><Edit /></el-icon> DockerCompose.yml</template>
          <div>
            <el-form>
              <el-form-item>
                <el-input v-model="ymlContent" type="textarea" rows="10"
                          placeholder="Define or paste the content of Your DockerCompose.yml here"></el-input>
              </el-form-item>
            </el-form>
          </div>
        </el-tab-pane>
        <div>
        <el-row>
          <el-col :span="2">
            <div class="action-group">
              <el-button @click="show_compose" type="primary" size="mini">展示</el-button>
            </div>
            </el-col>
            <el-col :span="22" style="margin-top: 1px">
              <div>
                <el-upload
                  ref="upload"
                  :http-request="upload1"
                  :max-size="2048"
                  action="/CombinationImage/"
                  :before-upload="beforeAvatarUpload1"
                  :on-remove="removeChange1"
                  :on-change="handleChange1"
                  :file-list="fileList">
                  <template #trigger>
                    <el-button style="margin-bottom: 20px" size="mini" type="primary">上传文件</el-button>
                  </template>
                </el-upload>
              </div>
            </el-col>
          </el-row>
        </div>
      </el-tabs>
    </el-dialog>
  </div>
</template>
<script setup>
import { ref, reactive, onMounted, nextTick } from 'vue'
import { useRouter, useRoute } from 'vue-router'
import { ElMessage, ElMessageBox } from 'element-plus'
import { Edit, Plus, Upload, ArrowLeft, Download } from '@element-plus/icons-vue'
import { layoutCreate, uploadImage, build_compose, show_build_status, uploadFile, deleteFile, upload_zip_file, layoutList } from '@/api/layout'
import connectorRules from '@/config/connectorRules'
import vTopoAttrPanel from './components/vTopoAttrPanel.vue'
import vShapebar from './components/vShapebar.vue'
import $ from 'jquery'

const props = defineProps({
  editable: {
    type: Boolean,
    default: true
  },
  layoutId: {
    type: String,
    default: ""
  }
})

const router = useRouter()
const route = useRoute()

// --- reactive complex state objects (mostly mutated in-place, never fully reassigned) ---
const svgAttr = reactive({ width: 0, height: 0, isHand: false, viewX: 0, viewY: 0, minW: 0, minH: 0, isCrosshair: false })
const svgToolbar = reactive([
  { name: '默认模式', className: 'toolbar-default', isActive: true },
  { name: '框选模式', className: 'toolbar-rectangle_selection', isActive: false }
])
const shapebarMoveNode = reactive({ left: 0, top: 0, name: '', icon: '', isShow: false })
const svgTopo = reactive({ isMoveover: false })
const selectionBox = reactive({ x: 0, y: 0, width: 0, height: 0, isShow: false })
const connectingLine = reactive({ x1: 0, y1: 0, x2: 0, y2: 0, isConnecting: true, sourceNode: '', endNode: '' })
const marker = reactive({ xmarkerY: 0, xmarkerX: 0, ymarkerX: 0, ymarkerY: 0, isMarkerShow: false })
const gridData = reactive([
  { x1: 0, x2: 100, y1: 20, y2: 20, color: '#c0c0c0', strokeWidth: 1, opacity: 0.3, id: 1 },
  { x1: 0, x2: 100, y1: 40, y2: 40, color: '#c0c0c0', strokeWidth: 1, opacity: 0.3, id: 2 },
  { x1: 0, x2: 100, y1: 60, y2: 60, color: '#c0c0c0', strokeWidth: 1, opacity: 0.3, id: 3 },
  { x1: 0, x2: 100, y1: 80, y2: 80, color: '#c0c0c0', strokeWidth: 1, opacity: 0.3, id: 4 },
  { x1: 20, x2: 20, y1: 0, y2: 100, color: '#c0c0c0', strokeWidth: 1, opacity: 0.3, id: 5 },
  { x1: 40, x2: 40, y1: 0, y2: 100, color: '#c0c0c0', strokeWidth: 1, opacity: 0.3, id: 6 },
  { x1: 60, x2: 60, y1: 0, y2: 100, color: '#c0c0c0', strokeWidth: 1, opacity: 0.3, id: 7 },
  { x1: 80, x2: 80, y1: 0, y2: 100, color: '#c0c0c0', strokeWidth: 1, opacity: 0.3, id: 8 },
  { x1: 100, x2: 100, y1: 0, y2: 100, color: '#c0c0c0', strokeWidth: 2, opacity: 0.6, id: 9 },
  { x1: 0, x2: 100, y1: 100, y2: 100, color: '#c0c0c0', strokeWidth: 2, opacity: 0.6, id: 10 }
])
const topoData = reactive({ nodes: [], connectors: [] })
const layout = reactive({ id: '', name: '', desc: '', imageName: '' })

// --- ref values (primitives or objects that get fully reassigned) ---
const selectNodeData = ref({})
const selectNodeIndex = ref(0)
const topoId = ref('')
const activeNames = ref(['1'])
const connectorWSelf = ref(15)
const connectorW = ref(15)
const containTop = ref(30)
const containLeft = ref(22)
const classchoose = ref(false)
const isShow = ref(true)
const editShow = ref(false)
const editLoading = ref(false)
const ymlContent = ref('')
const ymlShow = ref(false)
const fileList = ref([])
const newFile = ref(new FormData())
const newLayoutFile = ref(new FormData())

// --- Utility ---
function GenNonDuplicateID(randomLength) {
  return Number(Math.random().toString().substr(3, randomLength) + Date.now()).toString(36)
}

function canConnectorTo(curNodeType, connectorToNodeType, connectorType) {
  let canConnector = false
  if (connectorType === 'Link') {
    connectorRules.forEach((ele) => {
      if (ele.type === curNodeType) {
        ele.canLinkToType.forEach((el) => {
          if (el === connectorToNodeType) canConnector = true
        })
      }
    })
  } else if (connectorType === 'Contain') {
    connectorRules.forEach((ele) => {
      if (ele.type === curNodeType) {
        ele.canBeContainedType.forEach((el) => {
          if (el === connectorToNodeType) canConnector = true
        })
      }
    })
  }
  return canConnector
}

// --- Methods from original Vue 2 view ---
function dragShapeNode(nodeData, key, event) {
  let NODE = nodeData[key]
  let toolbarName = NODE.type
  let toolbarIcon = NODE.icon
  let topoEle = $(`#topoId${topoId.value}`)
  let svgOffsetLeft = topoEle.find('.topoSvg').offset().left
  let svgOffsetTop = topoEle.find('.topoSvg').offset().top
  let svgWidth = topoEle.find('.topoSvg').width()
  let svgHeight = topoEle.find('.topoSvg').height()
  let isContainSvgArea = false
  document.onmousemove = (event) => {
    let mouseX = event.clientX
    let mouseY = event.clientY
    let nodeX = event.clientX - svgOffsetLeft + $(document).scrollLeft() + svgAttr.viewX
    let nodeY = event.clientY - svgOffsetTop + $(document).scrollTop() + svgAttr.viewY
    isContainSvgArea = false
    shapebarMoveNode.left = mouseX + 4 + $(document).scrollLeft()
    shapebarMoveNode.top = mouseY + 4 + $(document).scrollTop()
    shapebarMoveNode.name = toolbarName
    shapebarMoveNode.icon = toolbarIcon
    shapebarMoveNode.isShow = true
    marker.isMarkerShow = false
    if (mouseX >= svgOffsetLeft &&
      mouseX <= (svgOffsetLeft + svgWidth) &&
      mouseY >= (svgOffsetTop - $(document).scrollTop()) &&
      mouseY <= (svgOffsetTop + svgHeight - $(document).scrollTop())
    ) {
      marker.isMarkerShow = true
      isContainSvgArea = true
      let n1 = Math.floor(nodeX / 20)
      let n2 = Math.floor(nodeY / 20)
      marker.xmarkerY = n2 * 20
      marker.ymarkerX = n1 * 20
    }
  }
  document.onmouseup = (event) => {
    document.onmousemove = null
    document.onmouseup = null
    if (isContainSvgArea) {
      let TOPODATA = topoData
      let type = NODE.type
      let name = NODE.type
      let id = GenNonDuplicateID(5)
      let nodeEndX = marker.ymarkerX
      let nodeEndY = marker.xmarkerY
      let svgNode = {
        name, type, id,
        x: nodeEndX, y: nodeEndY,
        icon: NODE.icon,
        width: NODE.width, height: NODE.height,
        initW: NODE.width, initH: NODE.height,
        classType: NODE.classType,
        isLeftConnectShow: false,
        isRightConnectShow: false,
        containNodes: [],
        attrs: {}
      }
      marker.isMarkerShow = false
      topoData.nodes.push(svgNode)
      for (let i = (TOPODATA.nodes.length - 1); i >= 0; i--) {
        let node = TOPODATA.nodes[i]
        if (node.x <= nodeEndX && nodeEndX <= (node.x + node.width) && nodeEndY >= node.y && node.y + node.height >= nodeEndY && node.id !== id) {
          let canBeContain = canConnectorTo(NODE.type, node.type, 'Contain')
          if (canBeContain) {
            let connectorId = GenNonDuplicateID(3)
            let connector = {
              id: connectorId,
              type: 'Contain',
              sourceNode: { id: id },
              targetNode: { id: node.id },
              isSelect: false
            }
            TOPODATA.connectors.push(connector)
            node.containNodes.push(id)
            refreshRowAndOuterNode(svgNode)
            refreshConnectorsData()
            break
          }
        }
      }
    }
    shapebarMoveNode.left = 0
    shapebarMoveNode.top = 0
    shapebarMoveNode.name = ''
    shapebarMoveNode.icon = ''
    shapebarMoveNode.isShow = false
  }
}

function mousedownTopoSvg(event) {
  let mouseX0 = event.clientX
  let mouseY0 = event.clientY
  let startViewX = svgAttr.viewX
  let startViewY = svgAttr.viewY
  let startSvgW = svgAttr.width
  let startSvgH = svgAttr.height
  let svgMinW = svgAttr.minW
  let svgMinH = svgAttr.minH
  let selectionBoxX = 0
  let selectionBoxY = 0
  cancelAllNodesSelect()
  cancelAllLinksSelect()
  if (svgToolbar[1].isActive) {
    let topoEle = $(`#topoId${topoId.value}`)
    selectionBoxX = event.clientX - topoEle.find('.topoSvg').offset().left + $(document).scrollLeft() + svgAttr.viewX
    selectionBoxY = event.clientY - topoEle.find('.topoSvg').offset().top + 4 + $(document).scrollTop() + svgAttr.viewY
    selectionBox.isShow = true
    selectionBox.x = selectionBoxX
    selectionBox.y = selectionBoxY
  }
  document.onmousemove = (event) => {
    let disX = event.clientX - mouseX0
    let disY = event.clientY - mouseY0
    let endSvgW = startSvgW - disX
    let endSvgH = startSvgH - disY
    if (svgToolbar[1].isActive) {
      let selectionW = Math.abs(disX)
      let selectionH = Math.abs(disY)
      svgAttr.isCrosshair = true
      if (disX <= 0) {
        selectionBox.x = selectionBoxX + disX
      } else {
        selectionBox.x = selectionBoxX
      }
      if (disY <= 0) {
        selectionBox.y = selectionBoxY + disY
      } else {
        selectionBox.y = selectionBoxY
      }
      selectionBox.width = selectionW
      selectionBox.height = selectionH
      return false
    }
    svgAttr.isHand = true
    svgAttr.viewX = (startViewX <= disX) ? 0 : startViewX - disX
    svgAttr.viewY = (startViewY <= disY) ? 0 : startViewY - disY
    svgAttr.width = (endSvgW < svgMinW) ? svgMinW : endSvgW
    svgAttr.height = (endSvgH < svgMinH) ? svgMinH : endSvgH
    marker.xmarkerX = svgAttr.width
    marker.ymarkerY = svgAttr.height
  }
  document.onmouseup = (event) => {
    document.onmousemove = null
    document.onmouseup = null
    svgAttr.isHand = false
    svgAttr.isCrosshair = false
    if (svgToolbar[1].isActive) {
      let sW = selectionBox.width
      let sH = selectionBox.height
      let sX = selectionBox.x
      let sY = selectionBox.y
      topoData.nodes.forEach((node) => {
        if (sX <= node.x && sY <= node.y && node.x + node.width <= sX + sW && node.y + node.height <= sY + sH) {
          node.isSelect = true
        }
      })
      selectionBox.isShow = false
      selectionBox.x = 0
      selectionBox.y = 0
      selectionBox.width = 0
      selectionBox.height = 0
    }
  }
}

function dragSvgNode(key, event) {
  if (!props.editable) return false
  let mouseX0 = event.clientX + $(document).scrollLeft()
  let mouseY0 = event.clientY + $(document).scrollTop()
  let CURNODE = topoData.nodes[key]
  let startX = CURNODE.x
  let startY = CURNODE.y
  let curNodeId = CURNODE.id
  let nodeW = CURNODE.width
  let nodeH = CURNODE.height
  let nodeStartPosArr = []
  marker.isMarkerShow = true
  topoData.nodes.splice(key, 1)
  topoData.nodes.push(CURNODE)
  putInnerNodeLast(CURNODE)
  cancelAllNodesSelect()
  cancelAllLinksSelect()
  CURNODE.isSelect = true
  storeCurnodeStartPosition(CURNODE, nodeStartPosArr)
  topoData.nodes.forEach((node) => {
    if (node.id === CURNODE.id) {
      selectNodeData.value = node
    }
  })
  document.onmousemove = (event) => {
    let disX = event.clientX - mouseX0 + $(document).scrollLeft()
    let disY = event.clientY - mouseY0 + $(document).scrollTop()
    let endX = startX + disX
    let endY = startY + disY
    let n1 = Math.floor(endX / 20)
    let n2 = Math.floor(endY / 20)
    if (n1 <= 0) n1 = 0
    if (n2 <= 0) n2 = 0
    if (endX <= 0) { endX = 0; disX = -startX }
    if (endY <= 0) { endY = 0; disY = -startY }
    marker.isMarkerShow = true
    marker.xmarkerY = n2 * 20
    marker.ymarkerX = n1 * 20
    moveContianNode(disX, disY, nodeStartPosArr)
    refreshConnectorsData()
  }
  document.onmouseup = (event) => {
    document.onmousemove = null
    document.onmouseup = null
    marker.isMarkerShow = false
    let NodeEndX = marker.ymarkerX
    let NodeEndY = marker.xmarkerY
    let disX = NodeEndX - startX
    let disY = NodeEndY - startY
    let mouseDisX = event.clientX - mouseX0
    let mouseDisY = event.clientY - mouseY0
    moveContianNode(disX, disY, nodeStartPosArr)
    drawContainLayout(CURNODE, NodeEndX, NodeEndY, true, nodeStartPosArr, mouseDisX, mouseDisY, startY)
    refreshConnectorsData()
  }
}

function drawContainLayout(CURNODE, NodeEndX, NodeEndY, isStop, nodeStartPosArr, mouseDisX, mouseDisY, startY) {
  let TOPODATA = topoData
  let curNodeId = CURNODE.id
  let nodeW = CURNODE.width
  let nodeH = CURNODE.height
  let originTargetNodeId = ''
  let originTargetNode = {}
  let NodePoint1 = [NodeEndX, NodeEndY]
  let NodePoint2 = [(NodeEndX + nodeW), NodeEndY]
  let NodePoint3 = [(NodeEndX + nodeW), (NodeEndY + nodeH)]
  let NodePoint4 = [NodeEndX, (NodeEndY + nodeH)]
  TOPODATA.connectors.forEach((ele) => {
    if (ele.type === 'Contain' && ele.sourceNode.id === curNodeId) {
      originTargetNodeId = ele.targetNode.id
    }
  })
  if (originTargetNodeId) {
    TOPODATA.nodes.forEach((node) => {
      if (node.id === originTargetNodeId) originTargetNode = node
    })
  }
  let endNodeY = startY + mouseDisY
  if (
    originTargetNode &&
    Math.abs(mouseDisX) <= containLeft.value &&
    endNodeY < originTargetNode.y + originTargetNode.height &&
    endNodeY > originTargetNode.y - CURNODE.height
  ) {
    refreshRowAndOuterNode(originTargetNode)
    return false
  }
  deleteCurNodeContainConnector(CURNODE)
  var isContainNode = false
  let overlapTargetNode = {}
  for (let i = (TOPODATA.nodes.length - 1); i >= 0; i--) {
    let targetNode = TOPODATA.nodes[i]
    isContainNode = false
    if (CURNODE.id !== targetNode.id) {
      let minX = targetNode.x
      let maxX = targetNode.x + targetNode.width
      let minY = targetNode.y
      let maxY = targetNode.y + targetNode.height
      let canContianTargetNode = canConnectorTo(CURNODE.type, targetNode.type, 'Contain')
      if (NodePoint1[0] <= maxX && NodePoint1[0] >= minX && NodePoint1[1] <= maxY && NodePoint1[1] >= minY) isContainNode = true
      if (NodePoint2[0] <= maxX && NodePoint2[0] >= minX && NodePoint2[1] <= maxY && NodePoint2[1] >= minY) isContainNode = true
      if (NodePoint4[0] <= maxX && NodePoint4[0] >= minX && NodePoint4[1] <= maxY && NodePoint4[1] >= minY) isContainNode = true
      if (NodePoint3[0] <= maxX && NodePoint3[0] >= minX && NodePoint3[1] <= maxY && NodePoint3[1] >= minY) isContainNode = true
      if (isContainNode && canContianTargetNode) {
        overlapTargetNode = targetNode
        break
      }
    }
  }
  if (isContainNode) {
    let connectorId = GenNonDuplicateID(3)
    let connector = {
      id: connectorId, type: 'Contain',
      sourceNode: { id: CURNODE.id },
      targetNode: { id: overlapTargetNode.id },
      isSelect: false
    }
    TOPODATA.connectors.push(connector)
    TOPODATA.nodes.forEach((node) => {
      if (node.id === overlapTargetNode.id) node.containNodes.push(CURNODE.id)
    })
    refreshRowAndOuterNode(CURNODE)
  }
  if (isContainNode) {
    nodeStartPosArr.forEach((node) => {
      if (node.id === CURNODE.id) {
        moveContianNode(CURNODE.x - node.x, CURNODE.y - node.y, nodeStartPosArr)
      }
    })
  }
  if (originTargetNodeId && originTargetNodeId !== overlapTargetNode.id) {
    refreshRowAndOuterNode(originTargetNode)
  }
}

function storeCurnodeStartPosition(CURNODE, startNodePosition) {
  let containNodes = CURNODE.containNodes
  startNodePosition.push({ id: CURNODE.id, x: CURNODE.x, y: CURNODE.y })
  if (containNodes.length) {
    containNodes.forEach((nodeId) => {
      topoData.nodes.forEach((ele) => {
        if (ele.id === nodeId) {
          storeCurnodeStartPosition(ele, startNodePosition)
        }
      })
    })
  }
}

function moveContianNode(disX, disY, nodeStartPosArr) {
  nodeStartPosArr.forEach((ele) => {
    let storeInfoId = ele.id
    topoData.nodes.forEach((node) => {
      if (node.id === storeInfoId) {
        node.x = ele.x + disX
        node.y = ele.y + disY
      }
    })
  })
}

function putInnerNodeLast(CURNODE) {
  let curNodeId = CURNODE.id
  topoData.connectors.forEach((ele) => {
    if (ele.type === 'Contain' && ele.targetNode.id === curNodeId) {
      let childNodeId = ele.sourceNode.id
      topoData.nodes.forEach((node, index) => {
        if (node.id === childNodeId) {
          let childNode = node
          topoData.nodes.splice(index, 1)
          topoData.nodes.push(childNode)
          putInnerNodeLast(childNode)
        }
      })
    }
  })
}

function deleteCurNodeContainConnector(CURNODE) {
  let curNodeId = CURNODE.id
  topoData.connectors.forEach((ele, key) => {
    if (ele.type === 'Contain' && ele.sourceNode.id === curNodeId) {
      let targetNodeId = ele.targetNode.id
      topoData.connectors.splice(key, 1)
      topoData.nodes.forEach((node) => {
        if (node.id === targetNodeId) {
          if (node.containNodes.length) {
            node.containNodes.forEach((ele2, idx) => {
              if (ele2 === curNodeId) {
                node.containNodes.splice(idx, 1)
              }
            })
          }
        }
      })
    }
  })
}

function refreshOuterNodeWidth(CURNODE) {
  topoData.connectors.forEach((ele) => {
    if (ele.sourceNode.id === CURNODE.id && ele.type === 'Contain') {
      let targetNodeId = ele.targetNode.id
      topoData.nodes.forEach((node) => {
        if (node.id === targetNodeId) {
          node.width = 2 * containLeft.value + CURNODE.width
          node.height = 10 + CURNODE.height + containTop.value
          refreshOuterNodeWidth(node)
        }
      })
    }
  })
}

function refreshRowAndOuterNode(TARGETNODE) {
  if (TARGETNODE.containNodes.length > 0) {
    let sumWidth = 0
    let maxHeight = 0
    TARGETNODE.containNodes.forEach((ele) => {
      let containNodeId = ele
      topoData.nodes.forEach((node) => {
        if (node.id === containNodeId) {
          sumWidth += node.width
          if (node.height > maxHeight) maxHeight = node.height
        }
      })
    })
    sumWidth += (TARGETNODE.containNodes.length + 1) * containLeft.value
    TARGETNODE.width = sumWidth
    TARGETNODE.height = maxHeight + 10 + containTop.value
  } else {
    TARGETNODE.width = TARGETNODE.initW
    TARGETNODE.height = TARGETNODE.initH
  }
  topoData.connectors.forEach((ele) => {
    if (ele.sourceNode.id === TARGETNODE.id && ele.type === 'Contain') {
      let parentNodeId = ele.targetNode.id
      topoData.nodes.forEach((node) => {
        if (node.id === parentNodeId) refreshRowAndOuterNode(node)
      })
    }
  })
  refreshContainNodesPosition(TARGETNODE)
}

function refreshContainNodesPosition(TARGETNODE) {
  TARGETNODE.containNodes.forEach((ele, key) => {
    let containNodeId = ele
    let containNode
    let preNode
    topoData.nodes.forEach((node) => {
      if (node.id === containNodeId) {
        containNode = node
      }
    })
    if (key === 0) {
      refreshRowNodesPosition(TARGETNODE, containNode, null)
    } else {
      let preNodeIndex = key - 1
      let preNodeId = TARGETNODE.containNodes[preNodeIndex]
      topoData.nodes.forEach((node) => {
        if (node.id === preNodeId) preNode = node
      })
      refreshRowNodesPosition(TARGETNODE, containNode, preNode)
    }
  })
}

function refreshRowNodesPosition(TARGETNODE, CURNODE, PRENODE) {
  if (PRENODE != null) {
    CURNODE.x = PRENODE.x + PRENODE.width + containLeft.value
  } else {
    CURNODE.x = TARGETNODE.x + containLeft.value
  }
  CURNODE.y = TARGETNODE.y + containTop.value
  refreshContainNodesPosition(CURNODE)
}

function refreshConnectorsData() {
  topoData.connectors.forEach((item) => {
    topoData.nodes.forEach((node) => {
      if (item.sourceNode.id === node.id) {
        item.sourceNode.width = node.width
        item.sourceNode.height = node.height
        item.sourceNode.x = node.x
        item.sourceNode.y = node.y
      }
      if (item.targetNode.id === node.id) {
        item.targetNode.width = node.width
        item.targetNode.height = node.height
        item.targetNode.x = node.x
        item.targetNode.y = node.y
      }
    })
  })
}

function drawConnectLine(key, event) {
  if (!props.editable) return false
  let CONNECTLINE = connectingLine
  let CURNODE = topoData.nodes[key]
  let nodeW = CURNODE.width
  let nodeH = CURNODE.height
  let sourceNodeX = CURNODE.x
  let sourceNodeY = CURNODE.y
  let mouseX0 = event.clientX
  let mouseY0 = event.clientY
  let topoEle = $(`#topoId${topoId.value}`)
  let x1 = event.clientX - topoEle.find('.topoSvg').offset().left - 2 + $(document).scrollLeft() + svgAttr.viewX
  let y1 = event.clientY - topoEle.find('.topoSvg').offset().top + 4 + $(document).scrollTop() + svgAttr.viewY
  CONNECTLINE.isConnecting = true
  CONNECTLINE.x1 = x1
  CONNECTLINE.y1 = y1
  CONNECTLINE.x2 = x1
  CONNECTLINE.y2 = y1
  CONNECTLINE.sourceNode = CURNODE.id
  document.onmousemove = (event) => {
    let disX = event.clientX - mouseX0
    let disY = event.clientY - mouseY0
    let x2 = x1 + disX
    let y2 = y1 + disY
    CURNODE.isRightConnectShow = true
    CONNECTLINE.x2 = x2
    CONNECTLINE.y2 = y2
  }
  document.onmouseup = () => {
    document.onmousemove = null
    document.onmouseup = null
    let hasConnected = false
    let CONNECTORS = topoData.connectors
    let sourceNodeW = nodeW
    let sourceNodeH = nodeH
    let targetNodeW = 0
    let targetNodeH = 0
    let targetNodeX = 0
    let targetNodeY = 0
    let targetNodeType = ''
    let connectType = ''
    if (CONNECTLINE.endNode) {
      CONNECTORS.forEach((item) => {
        if (item.sourceNode.id === CURNODE.id && item.targetNode.id === CONNECTLINE.endNode && item.type === 'Line') {
          hasConnected = true
        }
      })
      if (!hasConnected) {
        connectType = 'Line'
        topoData.nodes.forEach((item) => {
          if (item.id === CONNECTLINE.endNode) {
            targetNodeW = item.width
            targetNodeH = item.height
            targetNodeX = item.x
            targetNodeY = item.y
            targetNodeType = item.type
          }
        })
        let canLinkToTargetNode = canConnectorTo(CURNODE.type, targetNodeType, 'Link')
        if (!canLinkToTargetNode) {
          ElMessage({
            showClose: true,
            message: CURNODE.type + '类型 不能连接 ' + targetNodeType + '类型',
            type: 'error'
          })
          CURNODE.isRightConnectShow = false
          CONNECTORS.forEach((item) => {
            topoData.nodes.forEach((node) => {
              if (node.id === item.sourceNode.id && item.type === 'Line') node.isRightConnectShow = true
            })
          })
        } else {
          let connectorId = GenNonDuplicateID(3)
          let connector = {
            id: connectorId, type: connectType,
            strokeW: 3, color: '#768699',
            targetNode: { x: targetNodeX, y: targetNodeY, id: CONNECTLINE.endNode, width: targetNodeW, height: targetNodeH },
            sourceNode: { x: sourceNodeX, y: sourceNodeY, id: CURNODE.id, width: sourceNodeW, height: sourceNodeH }
          }
          CURNODE.isRightConnectShow = true
          topoData.nodes.forEach((item) => {
            if (item.id === CONNECTLINE.endNode) item.isLeftConnectShow = true
          })
          CONNECTORS.push(connector)
        }
      }
    } else {
      CURNODE.isRightConnectShow = false
      CONNECTORS.forEach((item) => {
        topoData.nodes.forEach((node) => {
          if (node.id === item.sourceNode.id && item.type === 'Line') node.isRightConnectShow = true
        })
      })
    }
    CONNECTLINE.x1 = 0
    CONNECTLINE.y1 = 0
    CONNECTLINE.x2 = 0
    CONNECTLINE.y2 = 0
    CONNECTLINE.isConnecting = false
    CONNECTLINE.sourceNode = ''
    CONNECTLINE.endNode = ''
  }
}

function mouseoverNode(key, event) {
  marker.xmarkerY = topoData.nodes[key].y
  marker.ymarkerX = topoData.nodes[key].x
  getConnectLine(key)
}

function getConnectLine(key) {
  connectingLine.endNode = topoData.nodes[key].id
}

function mouseoutLeftConnector(key) {
  connectingLine.endNode = ''
}

function selectConnectorLine(key) {
  if (!props.editable) return false
  let connectors = topoData.connectors
  let selectLine = topoData.connectors[key]
  let lastIndex = connectors.length - 1
  connectors.splice(key, 1)
  connectors.push(selectLine)
  cancelAllNodesSelect()
  cancelAllLinksSelect()
  selectLine.isSelect = true
  connectors[lastIndex] = selectLine
  selectNodeData.value = selectLine
}

function cancelAllNodesSelect() {
  topoData.nodes.forEach((ele) => {
    ele.isSelect = false
  })
  selectNodeData.value = {}
}

function cancelAllLinksSelect() {
  topoData.connectors.forEach((ele) => {
    ele.isSelect = false
  })
  selectNodeData.value = {}
}

function deleteNodeAndConnector() {
  document.onkeydown = (event) => {
    let paths = event.composedPath()
    let keycode = event.which
    if (paths.length > 10) {
      return
    }
    if (keycode === 46 || keycode === 8) {
      for (let i = 0; i < topoData.nodes.length; i++) {
        let node = topoData.nodes[i]
        if (node.isSelect) {
          deleteSelectNodeLink(node.id)
          let targetNodeId = ''
          topoData.connectors.forEach((ele) => {
            if (ele.sourceNode.id === node.id) targetNodeId = ele.targetNode.id
          })
          deleteCurNodeContainConnector(node)
          if (targetNodeId) {
            topoData.nodes.forEach((n) => {
              if (n.id === targetNodeId) refreshRowAndOuterNode(n)
            })
          }
          topoData.nodes.splice(i, 1)
          deleteCurnodeAndChildnodes(node)
          refreshNodeArrows()
          i--
          if (topoData.nodes.length > 0) {
            selectNodeIndex.value = 0
            selectNodeData.value = {}
          } else {
            selectNodeIndex.value = null
            selectNodeData.value = {}
          }
        }
      }
      topoData.connectors.forEach((ele, key) => {
        if (ele.isSelect) {
          topoData.connectors.splice(key, 1)
          refreshNodeArrows()
        }
      })
      refreshConnectorsData()
    }
  }
}

function deleteSelectNodeLink(selectId) {
  let connectorObjArr = topoData.connectors
  let connectorsLen = connectorObjArr.length
  for (let i = 0; i < connectorsLen; i++) {
    let connectorObj = connectorObjArr[i]
    if (connectorObj.type === 'Line' && (connectorObj.sourceNode.id === selectId || connectorObj.targetNode.id === selectId)) {
      topoData.connectors.splice(i, 1)
      i--
      connectorsLen--
    }
  }
}

function deleteCurnodeAndChildnodes(CURNODE) {
  deleteCurNodeContainConnector(CURNODE)
  if (CURNODE.containNodes.length) {
    CURNODE.containNodes.forEach((containNodeId) => {
      let containId = containNodeId
      topoData.nodes.forEach((ele, index) => {
        if (ele.id === containId) {
          topoData.nodes.splice(index, 1)
          deleteSelectNodeLink(containId)
          deleteCurnodeAndChildnodes(ele)
        }
      })
    })
  }
}

function refreshNodeArrows() {
  topoData.nodes.forEach((topoNode) => {
    topoNode.isLeftConnectShow = false
    topoNode.isRightConnectShow = false
  })
  topoData.connectors.forEach((ele) => {
    let sourceNodeId = ele.sourceNode.id
    let targetNodeId = ele.targetNode.id
    if (ele.type === 'Line') {
      topoData.nodes.forEach((topoNode) => {
        if (topoNode.id === targetNodeId) topoNode.isLeftConnectShow = true
        if (topoNode.id === sourceNodeId) topoNode.isRightConnectShow = true
      })
    }
  })
}

function selectToolbar(key) {
  svgToolbar.forEach((ele) => {
    ele.isActive = false
  })
  svgToolbar[key].isActive = true
}

function saveTopoJson() {
  editShow.value = true
}

function handleOk() {
  if (layout.name === '' || layout.name === null) {
    ElMessage({ message: '请输入环境名称', type: 'error' })
    return
  }
  if (layout.imageName === '' || layout.imageName === null || layout.imageName === '/images/') {
    ElMessage({ message: '请上传 banner 图', type: 'error' })
    return
  }
  if (layout.id !== null && layout.id !== undefined && layout.id !== "") {
    ElMessageBox.confirm('确认修改，修改会影响用户得分', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(() => {
      handleLayoutCreate()
    }).catch(() => {})
  } else {
    handleLayoutCreate()
  }
}

function handleLayoutCreate() {
  let imgName = layout.imageName.replace('/images/', "")
  let formData = new FormData()
  formData.set("id", layout.id)
  formData.set("data", JSON.stringify(topoData))
  formData.set("name", layout.name)
  formData.set("desc", layout.desc)
  formData.set("img", imgName)
  layoutCreate(formData).then(response => {
    let rsp = response.data
    let ok = rsp.code === 200 || rsp.status === 200
    if (ok) {
      if (layout.id !== null && layout.id !== undefined && layout.id !== "") {
        ElMessage({ message: "修改成功", type: 'success' })
      } else {
        ElMessage({ message: "创建成功", type: 'success' })
      }
      goBack()
    } else {
      ElMessage({ message: rsp.msg, type: 'error' })
    }
    editShow.value = false
  })
}

function handleCancel() {
  editShow.value = false
}

function beforeAvatarUpload(file) {
  if (file) {
    newFile.value.set("img", file)
  } else {
    return false
  }
}

function upload() {
  let data = newFile.value
  uploadImage(data).then(response => {
    let rsp = response.data
    let ok = rsp.code === 200 || rsp.status === 200
    if (ok) {
      ElMessage({ message: '上传成功', type: 'success' })
      layout.imageName = rsp.data
    } else {
      ElMessage({ message: rsp.msg, type: 'error' })
    }
  }).catch(() => {
    ElMessage({ message: "服务器内部错误", type: 'error' })
  })
}

function goBack() {
  router.push({ path: '/layout/manager' })
}

function initTopoWH() {
  nextTick(() => {
    let ele = `#topoId${topoId.value}`
    let topoW = $(ele).width()
    let topoH = $(ele).height()
    marker.xmarkerX = topoW
    marker.ymarkerY = topoH
    svgAttr.width = topoW
    svgAttr.height = topoH
    svgAttr.minW = topoW
    svgAttr.minH = topoH
  })
}

function viewYml() {
  ymlShow.value = true
}

function beforeAvatarUpload1(file) {
  if (file) {
    newFile.value.set("file", file)
  } else {
    return false
  }
}

function removeChange1(file, files) {
  ElMessageBox.confirm('此操作将永久删除该文件, 是否继续?', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(() => {
    let delFile = new FormData()
    delFile.set("file", file.name)
    deleteFile(delFile).then(response => {
      let data = response.data
      if (data.status === 200) {
        for (let i = 0; i < files.length; i++) {
          if (files[i] === file) {
            files.splice(i, 1)
          }
        }
        ElMessage({ type: 'success', message: '删除成功!' })
      } else {
        files.push(file)
        ElMessage({ type: 'error', message: '删除失败!' })
      }
    })
  }).catch(() => {
    files.push(file)
    ElMessage({ type: 'info', message: '已取消删除' })
  })
}

function handleChange1(file, files) {
  fileList.value = files
}

function upload1(file) {
  let size = file.file.size / 1024 / 1024
  if (size > 2) {
    ElMessage({ message: "文件大小必须小于2M", type: 'error' })
    fileList.value.pop()
  } else {
    let data = newFile.value
    uploadFile(data).then(response => {
      let rsp = response.data
      let ok = rsp.code === 200 || rsp.status === 200
      if (ok) {
        for (let i = 0; i < fileList.value.length; i++) {
          if (fileList.value[i].name.indexOf("../compose_file/") === -1) {
            fileList.value[i].name = "../compose_file/" + fileList.value[i].name
          }
        }
        ElMessage({ message: '上传成功', type: 'success' })
      } else {
        fileList.value.pop()
        ElMessage({ message: rsp.msg, type: 'error' })
      }
    }).catch(() => {
      fileList.value.pop()
      ElMessage({ message: "服务器内部错误", type: 'error' })
    })
  }
}

function show_compose() {}

function beforeAvatarUploadLayout(file) {
  if (file) {
    newLayoutFile.value.set('zip_file', file)
  } else {
    return false
  }
}

function uploadlayout() {
  upload_zip_file(newLayoutFile.value).then(response => {
    let data = response.data
    if (data.code === 400) {
      ElMessage({ message: data.msg, type: 'error' })
    }
    if (data.code === 200) {
      ElMessage({ message: '上传成功', type: 'success' })
      let id = data.layout_id
      layoutList(id).then(response => {
        let rsp = response.data
        let rows = {}
        rsp.results.forEach((info) => {
          rows = info
        })
        Object.assign(topoData, JSON.parse(rows.raw_content))
        layout.id = rows.layout_id
        layout.name = rows.layout_name
        layout.desc = rows.layout_desc
        layout.imageName = rows.image_name
        ymlContent.value = rows.yml_content
      })
    }
  })
}

onMounted(() => {
  if (route.query.layoutData !== null && route.query.layoutData !== undefined && route.query.layoutData.layout_id) {
    Object.assign(topoData, JSON.parse(route.query.layoutData.raw_content))
    layout.id = route.query.layoutData.layout_id
    layout.name = route.query.layoutData.layout_name
    layout.desc = route.query.layoutData.layout_desc
    layout.imageName = route.query.layoutData.image_name
    ymlContent.value = route.query.layoutData.yml_content
  }
  deleteNodeAndConnector()
  topoId.value = GenNonDuplicateID(5)
  initTopoWH()
})
</script>
<style scoped lang="less">
@svg-common-color: #768699;
@stroke-width: 2;
@stroke-select-width: 3;
@stroke-select-color: red;
@border-color: #aaaaaa;
@storke-dasharray: 5, 5;
@theme-color: #f3f3f3;
@theme-font-color: #525252;
.svgSelectClass {
  filter: url(#f1);
}

.topoHead {
  display: flex;
  justify-content: space-between;
  align-items: center;
  width: 100%;
  box-sizing: border-box;
  border: solid @border-color;
  border-width: 1px 1px 0;
  box-shadow: inset 0 1px 0 0 #fff;
}

/*svgHead工具栏*/
.svgHead {
  width: 100%;
  height: 40px;
  box-sizing: border-box;
  padding: 0 20px;
  display: flex;
  justify-content: space-between;
  align-items: center;
  background: #f3f3f3;
  border: solid @border-color;
  border-width: 1px 1px 0;
  box-shadow: inset 0 1px 0 0 #fff;

  .svgHeadItemLst {
    display: flex;

    .svgHeadItem {
      padding: 5px 10px;
      border: 1px solid @border-color;
      cursor: pointer;
      list-style: none;
      border-left-width: 0;

      &:hover {
        background-color: #ebebeb
      }

      &:first-child {
        border-left-width: 1px
      }

      &.active {
        background-color: #ebebeb;
        box-shadow: 2px 2px 1px #ccc inset
      }
    }

    .svgToolBarItem {
      font-size: 13px;
      color: @theme-font-color;
      padding: 5px 10px;
      border-radius: 2px;
      box-sizing: border-box;
      margin-left: 5px;
      cursor: pointer;
      -webkit-user-select: none;
      user-select: none;

      .svgToolBarTxt {
        margin-left: 2px;
      }
    }
  }
}

/*svgMain*/
.svgMain {
  height: 100%;
  min-height: calc(100vh - 140px);
  max-height: calc(100vh - 140px);
  box-sizing: border-box;
  display: flex;
  flex: 1;
}

/*移动的node*/
.shapeIcon {
  text-align: center;
  -webkit-user-select: none;
  user-select: none;

  .shapeIconImg {
    width: 28px;
    height: 28px;
    -webkit-user-select: none;
    user-select: none;
  }
}

.shapeName {
  font-size: 12px;
  text-align: center;
  padding: 0 5px;
  text-overflow: ellipsis;
  overflow: hidden;
  white-space: nowrap;
  -webkit-user-select: none;
  user-select: none;
  color: #000
}

.moveNode {
  position: absolute;
  border: 1px solid @svg-common-color;
  box-sizing: border-box;

  &.nodeMoveCss {
    width: 57px;
    height: 57px;
    background-color: #fff;
    -webkit-user-select: none;
    user-select: none;
    box-sizing: border-box;
    padding: 5px;
  }
}

/*svgMain右侧svg主体区域*/
.topoWrap {
  flex: 1;
  box-sizing: border-box;
  border: 1px solid @border-color;
  overflow: hidden;
  position: relative;
  background: #fff;

  .topoSvg {
    box-sizing: border-box;
    background-color: #fff;
    -webkit-user-select: none;
    user-select: none;

    &.hand {
      cursor: pointer
    }

    &.crosshair {
      cursor: crosshair;
    }
  }
}

/*svg 节点 连线样式*/
.marker {
  stroke: #3d7ed5;
  stroke-width: 1;
  display: none;

  &.isMarkerShow {
    display: block;
  }
}

.nodesG {
  -webkit-user-select: none;
  user-select: none;

  &.isSelect .reactClass {
    stroke-width: @stroke-select-width;
    .svgSelectClass;
  }

  &.isSelect .nodeName {
    font-weight: 500;
  }

  &.hoverShowConnectorArror:hover .connectorArror {
    display: block
  }

  .nodeImg {
    -webkit-user-select: none;
    user-select: none;
  }

  .nodeName {
    font-size: 12px;
    fill: @svg-common-color;
    -webkit-user-select: none;
    user-select: none;
    white-space: nowrap;
    text-overflow: ellipsis;
  }

  .reactClass {
    stroke-width: @stroke-width;
    stroke: @svg-common-color;
    fill: #fff;
    cursor: default;
  }

  .connectorArror {
    display: none;

    &.connector {
      display: block;
    }

    .circleColor {
      fill: @svg-common-color
    }
  }
}

.connectorsG {
  .connectorLine {
    fill: none;

    &.defaultStrokeColor {
      stroke: @svg-common-color;
    }

    &.defaultStrokeW {
      stroke-width: @stroke-width;
    }
  }

  &.active .connectorLine {
    .svgSelectClass;
  }
}
</style>
<style>
.avatar-uploader .el-upload {
  border: 1px dashed #d9d9d9;
  border-radius: 6px;
  cursor: pointer;
  position: relative;
  overflow: hidden;
}
.avatar-uploader .el-upload:hover {
  border-color: #409EFF;
}
.avatar-uploader-icon {
  font-size: 28px;
  color: #8c939d;
  width: 120px;
  height: 120px;
  line-height: 120px;
  text-align: center;
}
.avatar {
  width: 120px;
  height: 120px;
  display: block;
}
.el-collapse-item__header {
  -webkit-user-select: none;
  user-select: none;
}
</style>
