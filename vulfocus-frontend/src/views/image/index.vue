...
<script>
  ...
  import { ImageAdd, ImageDelete,ImageLocal,ImageLocalAdd,ImageShare,ImageDownload,ImageEdit, searchImage } from "@/api/image"
    ...
    methods:{
      ...
      querySearchAsync(queryString, cb) {
        ...
        if (queryString === null || queryString === "" || queryString.length === 0){
          ...
          searchImage(queryString).then(response => {
            this.summaries = []
            if(response.data.status === 200){
              let summariesList = response.data.data["summaries"]
      ...
      searchSummariesList(keyword){
        this.summaries = []
        searchImage(keyword).then(response => {
          this.summaries = []
          if(response.data.status === 200){
            let summariesList = response.data.data["summaries"]
            if (summariesList != null) {
